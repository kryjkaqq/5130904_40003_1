...
static bool parseLine(const std::string& original, DataStruct_t& outData)
{
  ...
  while (true) {
    ...
    if (startColon == std::string::npos) break;
    const size_t spacePos = inner.find(' ', startColon + 1);
    if (spacePos == std::string::npos) break;
    const size_t endColon = inner.find(':', spacePos + 1);
    if (endColon == std::string::npos) break;
    ...
    if (fieldName == "key1") {
      double val = 0.0;
      if (!parseDoubleLit(fieldValue, val)) return false;
      outData.key1_ = val;
      hasKey1 = true;
    }
    else if (fieldName == "key2") {
      unsigned long long val = 0ULL;
      if (!parseUllLit(fieldValue, val)) return false;
      outData.key2_ = val;
      hasKey2 = true;
    }
